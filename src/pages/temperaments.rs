...
pub fn Temperaments() -> Element {
    ...
    let save = move |_| {
...

        {
            ...
            if let Some(idx) = *edit_idx.read() {
                if let Some(t) = s.temperaments.get_mut(idx) {
                    t.name = name;
                    t.divisions = divisions;
                    t.period = period;
                }
            } else {
                s.temperaments.push(Temperament { name, divisions, period });
            }
        }
    ...
    let delete = {
        move |idx: usize| {
            {
                let mut s = APP_STATE.write();
                if idx < s.temperaments.len() {
                    let name = s.temperaments[idx].name.clone();
                    s.temperaments.remove(idx);
                    s.notation_systems.retain(|ns| ns.temperament_name != name);
                    s.tunings.retain(|tu| tu.temperament_name != name);
                    s.scales.retain(|sc| sc.temperament_name != name);
                    s.chords.retain(|ch| ch.temperament_name != name);
                    if s.instruments.get(s.selected_instrument_idx.unwrap_or(usize::MAX))
                        .map(|i| i.temperament_name.as_str()) == Some(name.as_str())
                    {
