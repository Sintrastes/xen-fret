...
///
/// Struct containing all of the information that comes pre-loaded
///  with xen-fret.
///
/// For each temperament, it describes the set of available chords,
///  tunings, scales, and notation systems.
///
...
pub fn default_temperaments() -> Vec<Temperament> {
    TemperamentBundle::default_bundles().into_iter().map(|b| b.temperament).collect()
}

pub fn default_notation_systems() -> Vec<NotationSystem> {
    TemperamentBundle::default_bundles().into_iter().flat_map(|b| b.notation_systems).collect()
}

pub fn default_tunings() -> Vec<Tuning> {
    TemperamentBundle::default_bundles().into_iter().flat_map(|b| b.tunings).collect()
}

pub fn default_scales() -> Vec<Scale> {
    TemperamentBundle::default_bundles().into_iter().flat_map(|b| b.scales).collect()
}

pub fn default_chords() -> Vec<Chord> {
    TemperamentBundle::default_bundles().into_iter().flat_map(|b| b.chords).collect()
}
