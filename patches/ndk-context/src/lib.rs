...
pub unsafe fn initialize_android_context(java_vm: *mut c_void, context_jobject: *mut c_void) {
    // No assertion: Android can call onCreate again (activity re-creation after a config
    // change, or re-launch from back stack) without a preceding onDestroy. Replacing is safe.
    ANDROID_CONTEXT.replace(AndroidContext {
        java_vm,
        context_jobject,
    });
}
