...
use xen_theory::instrument::{default_instrument_types, default_string_count, suggest_tuning, Instrument};
                ...
                for (idx, name, itype, temp_name, num_strings) in &instruments {
                    div {
                        ...
                        onclick: {
                            let idx = *idx;
                            move |_| {
                                APP_STATE.write().select_instrument(idx);
        ...
        Modal {
            ...
            on_confirm: move |_| {
                if let Some(del_idx) = *pending_delete.read() {
                    APP_STATE.write().delete_instrument(del_idx);
                    app_common::storage::save(&*APP_STATE.read());
                }
    ...
    let save = move |_| {
        ...
        let instrument = Instrument {
            ...
            fret_markers: if *form_frets.read() >= 24 {
                xen_theory::tuning::guitar_markers()
            } else {
                vec![]
            },
    ...
    let save = move |_| {
        if let Some(idx) = *edit_idx.read() {
            ...
            let fret_markers = {
                ...
                let existing = s.instruments.get(idx).map(|i| i.fret_markers.clone()).unwrap_or_default();
                if existing.is_empty() && *form_frets.read() >= 24 {
                    xen_theory::tuning::guitar_markers()
                } else {
                    existing
                }
            ...
            {
                let mut s = APP_STATE.write();
                s.update_instrument(idx, updated);
                // Keep default_instrument preference in sync if this is the selected instrument.
